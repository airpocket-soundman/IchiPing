...
│   │   ├── mcux_include.json             ARMGCC/SDK パス（共通、開発機固有）
...
│   ├── 03_ili9341_test/                  TFT 配線確認＋5 フェーズ描画（LPSPI3 + 4 GPIO）
│   ├── 04_lvgl_test/                     LVGL v9 ＋ IchiPing 状態 UI モック（LPSPI3 + 4 GPIO）
...
| 6 | [`projects/06_mic_test`](projects/06_mic_test/) | INMP441 単体疎通 — PRINTF だけで RMS/Peak/ZCR 観測 | `sai/sai_interrupt_record` |
| 7 | [`projects/07_speaker_test`](projects/07_speaker_test/) | MAX98357A 単体疎通 — 200/1k/5k Hz + chirp 再生 | `sai/sai_interrupt_play` |
| 8 | [`projects/08_mic_speaker_test`](projects/08_mic_speaker_test/) | TX chirp ＋ RX キャプチャの閉ループ（インパルス応答） | `sai/sai_*` + `lpuart` |
| 9 | [`projects/09_audio_stream`](projects/09_audio_stream/) | 実マイク連続ストリーミング（01 の real audio 版） | `sai/sai_*` + `lpuart` |
| 10 | [`projects/10_collector`](projects/10_collector/) | PC 双方向制御の学習データ採取（窓 / 励起 / 反復 / ラベル） | `sai/sai_*` + `lpuart` ＋ PC `collector_client.py` |
...
1. **MCUXpresso SDK for FRDM-MCXN947** 11.9 以降を `d:/GitHub/mcuxsdk` 等に配置（`mcux_include.json` の `SdkRootDirPath` と一致させる）
2. **arm-gnu-toolchain 14.2** を `~/.mcuxpressotools/...` に展開（`mcux_include.json` の `ARMGCC_DIR` と一致）
3. VS Code で *MCUXpresso → Import Project From Folder* → `firmware/projects/XX_*/` を指定
...
| 06_mic_test | [README.md](projects/06_mic_test/README.md) | — | シリアルモニタで PRINTF 統計を眺める（PC スクリプト不要） |
| 07_speaker_test | [README.md](projects/07_speaker_test/README.md) | — | スピーカで耳判定（PC スクリプト不要） |
| 08_mic_speaker_test | [README.md](projects/08_mic_speaker_test/README.md) | — | **`receiver.py` 起動必須**。captures に room IR WAV |
| 09_audio_stream | [README.md](projects/09_audio_stream/README.md) | — | **`receiver.py` 起動必須**。実マイク音を連続記録 |
| 10_collector | [README.md](projects/10_collector/README.md) | — | **`collector_client.py` で PC 側から制御** |
