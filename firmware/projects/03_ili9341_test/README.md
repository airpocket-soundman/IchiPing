...
| **SDI / MOSI** | D11 | **J2.8** | P0_24 | Alt2 (FC1) | `FC1_SPI_SDO` | **SJ7: 1-2** (default) | LPSPI1 master out、microSD と共有可 |
...
| SDO / MISO | D12 | J2.10 | P0_26 | Alt2 (FC1) | `FC1_SPI_SDI` | — | ILI は書込専用なので未接続でも OK。pin_mux では muxed しておく（後で microSD 用） |
...
>
> **重要 3**: D10 (P0_27 = `FC1_SPI_PCS`) は SPI のハードウェア CS だが、本プロジェクトでは **A2 (P0_14) をマニュアル CS** として使うので D10 は muxed せず GPIO のまま。これにより SJ6 が default 1-2 のままで LED_GREEN がオンボードで点灯可能。後で microSD を追加するときは D10 を CS にすればハードウェア PCS が使える（ただし LED_GREEN は失う）。
