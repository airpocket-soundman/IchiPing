...
def main(argv=None) -> int:
    ...
    ap.add_argument("--clim", type=float, default=DIVERGE_CLIM,
                    help="帯カラーチャートの色域 (±dB)")
    ap.add_argument("--order", choices=("h", "class"), default="h",
                    help="行順: h=h 表記昇順 / class=等価クラス順")
    args = ap.parse_args(argv)

    if args.order == "class":
        # 等価クラス判定は s 表記前提なので s でソートし、表示だけ h にする
        s_sorted = sort_by_class(["s" + format(v, "05b") for v in range(32)])
    ...
    for i in range(1, n):
        # クラス順時はクラス境界を太線で強調
        boundary = row_tags is not None and row_tags[i] != row_tags[i - 1]
        ax.axhline(i, color="#1a1d23" if boundary else "#444444",
                   lw=1.4 if boundary else 0.4)
